    ...
    public static class FontGenerationServices
    {
        ...
        public static FontAtlasResult BuildAtlas(FontAtlasRequest request, IProgress<FontProgress> progress)
        {
                ...
                out sizeYIndex))
            {
                ...
                {
                    Success = false,
                    FailureMessage = CreateLayoutFailureMessage(fonts, request.CandidateWidths, request.CandidateHeights)
                };
            }
            layoutWatch.Stop();
            request.PerformanceStats?.Add("AtlasLayout", layoutWatch.Elapsed);
        ...
        public static bool TryLayoutFonts(List<Fnt_char> fonts, Size size, int gap, bool vertical, List<Rectangle> placements)
        {
            ...
            foreach (Fnt_char fnt in fonts)
            {
                ...
                Size glyphSize = GetAtlasGlyphSize(fnt);
        ...
        private static string CreateLayoutFailureMessage(
            List<Fnt_char> fonts,
            IList<int> candidateWidths,
            IList<int> candidateHeights)
        {
            int maxTexWidth = GetMaxCandidate(candidateWidths);
            int maxTexHeight = GetMaxCandidate(candidateHeights);
            int maxGlyphWidth = 0;
            ...
            return "Font file size exceeds the limit! Can not be processed. "
                + "Glyphs=" + fonts.Count
                + ", MaxTex=" + maxTexWidth + "x" + maxTexHeight
                + ", MaxGlyphW=" + maxGlyphWidth + "(" + widestHex + ")"
                + ", MaxGlyphH=" + maxGlyphHeight + "(" + tallestHex + ")"
                + ", TotalGlyphArea=" + totalArea.ToString("N0") + ".";
        }

        private static int GetMaxCandidate(IList<int> candidates)
        {
            int max = 0;
            if (candidates == null)
            {
        ...
        private static Size GetAtlasGlyphSize(Fnt_char fnt)
        {
            if (fnt == null || !fnt.Enable)
            {
                return Size.Empty;
            ...
            int width = CeilingPositive(fnt.fWidth);
            int height = CeilingPositive(fnt.fHeight);
            return width > 0 && height > 0 ? new Size(width, height) : Size.Empty;
        }

        private static int CeilingPositive(float value)
        {
